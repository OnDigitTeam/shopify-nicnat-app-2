...
/**
 * Central, validated config. Mirrors the constants the WooCommerce plugin used
 * (NICNAT_API_BASE, the get-shipping-rates endpoint, the method list, etc).
 */
...
export const config = {
  ...
  shopify: {
    ...
    // Optional: a long-lived offline admin token, handy for registering the
    // carrier service before the full OAuth flow is wired up.
    adminToken: process.env.SHOPIFY_ADMIN_TOKEN || "",
  },

  nicnat: {
    // ensure exactly one trailing slash, like rtrim(..,'/').'/' in the PHP client
